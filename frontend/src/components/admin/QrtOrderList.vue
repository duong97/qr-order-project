...
<template>
    <div id="order-list" class="p-2">
        ...
        <!-- Empty component -->
        <Empty v-if="!orderStore.orders.length" description="Chưa có đơn hàng nào" />

        <!-- Show danh sách order -->
        <div v-else>
            <TransitionGroup name="list" tag="div">
                <div v-for="(order, orderIndex) in orderStore.orders" :key="order.id + '_' + order.version" class="mb-5">
                    <CellGroup>
